...
import com.smart.cloud.fire.mvp.alarmPushAccount.AlarmPushAccountActivity;
import com.smart.cloud.fire.mvp.modifyBoundEmail.ModifyBoundEmailActivity;
...
public class AlarmSettingActivity extends MvpActivity<AlarmSettingPresenter> implements AlarmSettingView,View.OnClickListener {
    ...
    private AlarmSettingPresenter alarmSettingPresenter;
    ...
    @Override
    protected AlarmSettingPresenter createPresenter() {
        alarmSettingPresenter = new AlarmSettingPresenter(this);
        return alarmSettingPresenter;
    }
    ...
    public void regFilter() {
        ...
        filter.addAction(ConstantValues.P2P.ACK_RET_GET_ALARM_EMAIL);
        filter.addAction(ConstantValues.P2P.RET_GET_ALARM_EMAIL_WITHSMTP);

        filter.addAction(ConstantValues.P2P.ACK_RET_SET_MOTION);
        filter.addAction(ConstantValues.P2P.RET_SET_MOTION);
        filter.addAction(ConstantValues.P2P.RET_GET_MOTION);

        filter.addAction(ConstantValues.P2P.ACK_RET_SET_BUZZER);
        filter.addAction(ConstantValues.P2P.RET_SET_BUZZER);
        filter.addAction(ConstantValues.P2P.RET_GET_BUZZER);
        filter.addAction(ConstantValues.P2P.RET_GET_INFRARED_SWITCH);
        filter.addAction(ConstantValues.P2P.ACK_RET_SET_INFRARED_SWITCH);
        filter.addAction(ConstantValues.P2P.RET_GET_WIRED_ALARM_INPUT);
        filter.addAction(ConstantValues.P2P.RET_GET_WIRED_ALARM_OUT);
        filter.addAction(ConstantValues.P2P.ACK_RET_SET_WIRED_ALARM_INPUT);
        filter.addAction(ConstantValues.P2P.ACK_RET_SET_WIRED_ALARM_OUT);
        ...
        public void onReceive(Context arg0, Intent intent) {
            if (intent.getAction().equals(ConstantValues.P2P.RET_GET_BIND_ALARM_ID)) {
                ...
                if (result == ConstantValues.P2P_SET.BIND_ALARM_ID_SET.SETTING_SUCCESS) {
                    ...
                    ConstantValues.P2P.RET_GET_ALARM_EMAIL)) {
                // isRET=true;
                String email = intent.getStringExtra("email");
                int result = intent.getIntExtra("result", 0);
                getSMTPMessage(result);
                if (email.equals("") || email.equals("0")) {
                    email_text.setText("未绑定");
                } else {
                    email_text.setText(email);
                }
                showEmailState();
            } else if (intent.getAction().equals(
                    ConstantValues.P2P.RET_GET_ALARM_EMAIL_WITHSMTP)) {
                // isRET=true;
                ...
                if (contectid != null && contectid.equals(idOrIp)) {
                    ...
                    ConstantValues.P2P.RET_SET_ALARM_EMAIL)) {

            } else if (intent.getAction().equals(ConstantValues.P2P.RET_GET_MOTION)) {
                int state = intent.getIntExtra("motionState", -1);
                if (state == ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_ON) {
                    motion_switch = ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_ON;
                    motion_img.setBackgroundResource(R.mipmap.ic_checkbox_on);
                } else {
                    motion_switch = ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_OFF;
                    motion_img
                            .setBackgroundResource(R.mipmap.ic_checkbox_off);
                }
                showMotionState();
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_SET_MOTION)) {
                int result = intent.getIntExtra("result", -1);
                if (result == ConstantValues.P2P_SET.MOTION_SET.SETTING_SUCCESS) {
                    if (cur_modify_motion_state == ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_ON) {
                        motion_switch = ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_ON;
                        motion_img
                                .setBackgroundResource(R.mipmap.ic_checkbox_on);
                    } else {
            ...
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_GET_BUZZER)) {
                int state = intent.getIntExtra("buzzerState", -1);
                updateBuzzer(state);
                showBuzzerTime();
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_SET_BUZZER)) {
                int result = intent.getIntExtra("result", -1);
                if (result == ConstantValues.P2P_SET.BUZZER_SET.SETTING_SUCCESS) {
                ...
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_PWD_ERROR) {
                    ...
                    ConstantValues.P2P.ACK_RET_GET_ALARM_EMAIL)) {
                int result = intent.getIntExtra("result", -1);
                // if(isRET){
                ...
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_PWD_ERROR) {
                    ...
                    ConstantValues.P2P.ACK_RET_SET_MOTION)) {
                int result = intent.getIntExtra("result", -1);
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_PWD_ERROR) {
                    Intent i = new Intent();
                    i.setAction(ConstantValues.Action.CONTROL_SETTING_PWD_ERROR);
                    ...
                    ConstantValues.P2P.ACK_RET_SET_BUZZER)) {
                int result = intent.getIntExtra("result", -1);
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_PWD_ERROR) {
                    Intent i = new Intent();
                    i.setAction(ConstantValues.Action.CONTROL_SETTING_PWD_ERROR);
                    ...
                    ConstantValues.P2P.RET_GET_INFRARED_SWITCH)) {
                int state = intent.getIntExtra("state", -1);
                if (state == ConstantValues.P2P_SET.INFRARED_SWITCH.INFRARED_SWITCH_ON) {
                ...
                } else if (state == ConstantValues.P2P_SET.INFRARED_SWITCH.INFRARED_SWITCH_OFF) {
                    change_pir.setVisibility(RelativeLayout.VISIBLE);
                ...
                showImg_infrared_switch();
                    ...
                    ConstantValues.P2P.ACK_RET_SET_INFRARED_SWITCH)) {
                int result = intent.getIntExtra("result", -1);
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_NET_ERROR) {
                ...
                } else if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_SUCCESS) {
                    if (current_infrared_state == true) {
                        current_infrared_state = false;
                        pir_img.setBackgroundResource(R.mipmap.ic_checkbox_on);
                    } else {
                    ...
                    ConstantValues.P2P.RET_GET_WIRED_ALARM_INPUT)) {
                int state = intent.getIntExtra("state", -1);
                if (state == ConstantValues.P2P_SET.WIRED_ALARM_INPUT.ALARM_INPUT_ON) {
                    alarm_input_switch.setVisibility(RelativeLayout.VISIBLE);
                    isOpenWriedAlarmInput = false;
                    img_alarm_input
                            .setBackgroundResource(R.mipmap.ic_checkbox_on);
                } else if (state == ConstantValues.P2P_SET.WIRED_ALARM_INPUT.ALARM_INPUT_OFF) {
                    alarm_input_switch.setVisibility(RelativeLayout.VISIBLE);
                ...
                showImg_wired_alarm_input();
                    ...
                    ConstantValues.P2P.RET_GET_WIRED_ALARM_OUT)) {
                int state = intent.getIntExtra("state", -1);
                if (state == ConstantValues.P2P_SET.WIRED_ALARM_OUT.ALARM_OUT_ON) {
                    alarm_out_switch.setVisibility(RelativeLayout.VISIBLE);
                    isOpenWriedAlarmOut = false;
                    img_alarm_out
                            .setBackgroundResource(R.mipmap.ic_checkbox_on);
                } else if (state == ConstantValues.P2P_SET.WIRED_ALARM_OUT.ALARM_OUT_OFF) {
                    alarm_out_switch.setVisibility(RelativeLayout.VISIBLE);
                ...
                showImg_wired_alarm_out();
                    ...
                    ConstantValues.P2P.ACK_RET_SET_WIRED_ALARM_INPUT)) {
                ...
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_NET_ERROR) {
                    ...
                    ConstantValues.P2P.ACK_RET_SET_WIRED_ALARM_OUT)) {
                int result = intent.getIntExtra("state", -1);
                if (result == ConstantValues.P2P_SET.ACK_RESULT.ACK_NET_ERROR) {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.change_buzzer:
                showProgress();
                if (buzzer_switch != ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_OFF) {
                    cur_modify_buzzer_state = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_OFF;
                } else {
                    cur_modify_buzzer_state = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_ONE_MINUTE;
                }
            ...
            case R.id.change_motion:
                showProgress_motion();
                if (motion_switch != ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_OFF) {
                    cur_modify_motion_state = ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_OFF;
                    P2PHandler.getInstance().setMotion(idOrIp,
                            contact.contactPassword, cur_modify_motion_state);
                } else {
                    cur_modify_motion_state = ConstantValues.P2P_SET.MOTION_SET.MOTION_DECT_ON;
                    P2PHandler.getInstance().setMotion(idOrIp,
                            contact.contactPassword, cur_modify_motion_state);
                }
                break;
            ...
            case R.id.radio_two:
                showProgress();
                cur_modify_buzzer_state = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_TWO_MINUTE;
                P2PHandler.getInstance().setBuzzer(idOrIp, contact.contactPassword,
                        cur_modify_buzzer_state);
                break;
            case R.id.radio_three:
                showProgress();
                cur_modify_buzzer_state = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_THREE_MINUTE;
                P2PHandler.getInstance().setBuzzer(idOrIp, contact.contactPassword,
                        cur_modify_buzzer_state);
                break;
            case R.id.change_pir:
                ...
                if (current_infrared_state == true) {
                    modify_infrared_state = ConstantValues.P2P_SET.INFRARED_SWITCH.INFRARED_SWITCH_ON;
                    P2PHandler.getInstance().setInfraredSwitch(idOrIp,
                            contact.contactPassword, modify_infrared_state);
                } else {
                    modify_infrared_state = ConstantValues.P2P_SET.INFRARED_SWITCH.INFRARED_SWITCH_OFF;
                    P2PHandler.getInstance().setInfraredSwitch(idOrIp,
                            contact.contactPassword, modify_infrared_state);
                }
                break;
            ...
            case R.id.layout_alarm_switch:
                ...
                if (isReceiveAlarm == true) {
                    if (last_bind_data.length >= max_alarm_count) {
                        T.showShort(mContext,"报警接收人数已达上限");
                        showImg_receive_alarm();
                        return;
                    }
    ...
    public void updateBuzzer(int state) {
        if (state == ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_ONE_MINUTE) {
            buzzer_switch = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_ONE_MINUTE;
        ...
        } else if (state == ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_TWO_MINUTE) {
            buzzer_switch = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_TWO_MINUTE;
            buzzer_img.setBackgroundResource(R.mipmap.ic_checkbox_on);
            buzzer_time.setVisibility(RelativeLayout.VISIBLE);
            radio_two.setChecked(true);
        } else if (state == ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_THREE_MINUTE) {
            buzzer_switch = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_ON_THREE_MINUTE;
            buzzer_img.setBackgroundResource(R.mipmap.ic_checkbox_on);
            buzzer_time.setVisibility(RelativeLayout.VISIBLE);
            radio_three.setChecked(true);
        } else {
            buzzer_switch = ConstantValues.P2P_SET.BUZZER_SET.BUZZER_SWITCH_OFF;
