...
import com.smart.cloud.fire.adapter.MainFragmentAdapter;
import com.smart.cloud.fire.global.ConstantValues;
import com.smart.cloud.fire.global.Contact;
import com.smart.cloud.fire.global.MyApp;
import com.smart.cloud.fire.global.NpcCommon;
import com.smart.cloud.fire.mvp.alarmSetting.AlarmSettingActivity;
import com.smart.cloud.fire.mvp.defenceList.DefenceListActivity;
import com.smart.cloud.fire.mvp.fragment.MonitorOne.MonitorOneFragment;
import com.smart.cloud.fire.mvp.fragment.MonitorTwo.MonitorTwoFragment;
import com.smart.cloud.fire.mvp.modifyCameraInfo.ModifyCameraNameActivity;
...
import com.smart.cloud.fire.yoosee.AppConfig;
...
public class ApMonitorActivity extends BaseMonitorActivity implements ViewPager.OnPageChangeListener {
    ...
    public void initSpeark(int deviceType, boolean isOpenDor) {
        ...
        new_monitor.setAction(ConstantValues.Action.NEW_MONITOR);
    ...
    public void regFilter() {
        ...
        filter.addAction(ConstantValues.P2P.RET_GET_FOCUS_ZOOM);
        filter.addAction(ConstantValues.P2P.RET_GET_FOCUS_ZOOM_POSITION);
        filter.addAction(ConstantValues.P2P.RET_SET_FOCUS_ZOOM_POSITION);
        filter.addAction(ConstantValues.P2P.RET_TOSEE_PRESETMOTOROS);
    ...
    BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context arg0, Intent intent) {
            // TODO Auto-generated method stub
            if (intent.getAction().equals(ConstantValues.P2P.RET_TOSEE_PRESETMOTOROS)){
...
//                mOrientationEventListener.enable();
                ...
                isShake = false;
                    ...
                    ConstantValues.P2P.RET_GET_REMOTE_DEFENCE)) {
            ...
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_SET_REMOTE_DEFENCE)) {
                int result = intent.getIntExtra("state", -1);
                if (result == 0) {
                    if (defenceState == ConstantValues.DefenceState.DEFENCE_STATE_ON) {
                        defenceState = ConstantValues.DefenceState.DEFENCE_STATE_OFF;
                    } else {
                        defenceState = ConstantValues.DefenceState.DEFENCE_STATE_ON;
                    }
                    changeDefence(defenceState);
                }
...
//					 iv_full_screen.setVisibility(View.VISIBLE);
                    ...
                    ConstantValues.P2P.DELETE_BINDALARM_ID)) {
            ...
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_GET_FOCUS_ZOOM_POSITION)) {
            ...
            } else if (intent.getAction().equals(ConstantValues.P2P.RET_SET_FOCUS_ZOOM_POSITION)) {
                String deviceId = intent.getStringExtra("deviceId");
                int result = intent.getIntExtra("result", -1);
                int value = intent.getIntExtra("value", -1);
                if (deviceId.equals(callId)) {
    ...
    public void changeDefence(int defencestate) {
        ...
        Intent i = new Intent();
        i.setAction(ConstantValues.Action.CHANGE_REMOTE_DEFENCE);
    ...
    public void setDefence() {
        ...
        if (defenceState == ConstantValues.DefenceState.DEFENCE_STATE_ON) {
            P2PHandler.getInstance().setRemoteDefence(callId, password,
                    ConstantValues.P2P_SET.REMOTE_DEFENCE_SET.ALARM_SWITCH_OFF);

        } else if (defenceState == ConstantValues.DefenceState.DEFENCE_STATE_OFF) {
            P2PHandler.getInstance().setRemoteDefence(callId, password,
                    ConstantValues.P2P_SET.REMOTE_DEFENCE_SET.ALARM_SWITCH_ON);
        }
    }
...
//            P2PHandler.getInstance().call(NpcCommon.mThreeNum, password, true, ConstantValues.P2P_TYPE.P2P_TYPE_MONITOR, callId, ipAdress, push_mesg, AppConfig.VideoMode, mContact.contactId);
        }
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 1:
                    ...
                    playReady();
                    mContact.apModeState = ConstantValues.APmodeState.LINK;
                    break;
            }
        }
    };
...
//			setIsLand(true);
            ...
            pView.fullScreen();
            isFullScreen = true;
            LinearLayout.LayoutParams parames = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT);
            r_p2pview.setLayoutParams(parames);
        } else {
            ScrrenOrientation = Configuration.ORIENTATION_PORTRAIT;
...
//        }
    ...
    */
    ...
    public void setControlButtomHeight(int height) {
        LinearLayout.LayoutParams control_bottom_parames = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    ...
    public void setZoomMargin(boolean isFullScreen) {
        LinearLayout.LayoutParams zoom_params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        if (isFullScreen) {
