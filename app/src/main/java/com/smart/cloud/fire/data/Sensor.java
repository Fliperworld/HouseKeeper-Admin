package com.smart.cloud.fire.data;
...
//    }
     ...
     */
    public boolean getSensorSwitch(){
    	if (SensorCategory == SENSORCATEGORY_NORMAL && SensorType == ConstantValues.SensorType.TYPE_REMOTE_CONTROLLER)
            return false;
        int[] home = Utils.getByteBinnery(SensorData[4], false);
        int[] out = Utils.getByteBinnery(SensorData[5], false);
        int[] sleep = Utils.getByteBinnery(SensorData[6], false);
    ...
    public boolean isControlSensor() {
        if (SensorCategory == SENSORCATEGORY_NORMAL && (SensorType == ConstantValues.SensorType.TYPE_JACK
                || SensorType == ConstantValues.SensorType.TYPE_LIGHT
                || SensorType == ConstantValues.SensorType.TYPE_CURTAIN
                || SensorType == ConstantValues.SensorType.TYPE_WARNING_SPEAKER)) {
            return true;
        }
        return false;
    }
